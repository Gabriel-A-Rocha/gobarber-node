...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default('You cannot create an appointment on a past date.');
    }

    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default('You can only create appointments between 8:00 a.m. and 5:00 p.m.');
    }
    ...
    }); // format documentation can be found at: https://date-fns.org/v2.15.0/docs/format

    const dateFormatted = (0, _dateFns.format)(appointmentDate, "EEEE, dd/MM/yyyy 'at' HH:mm'h'");
    const notification = await this.notificationsRepository.create({
      recipient_id: provider_id,
      content: `An appointment has been scheduled on ${dateFormatted}`
    });

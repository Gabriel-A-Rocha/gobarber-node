...
let CreateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const hashedPassword = await this.hashProvider.generateHash(password); //create and save user

    const user = await this.usersRepository.create({
      name,
      email,
      password: hashedPassword
    }); // delete providers cache, so the list is updated in the next request
